...
class EmulatorThread(threading.Thread):
    def __init__(self, rom_path, speed):
        threading.Thread.__init__(self, daemon=True)
        self.pyboy = PyBoy(rom_path, window_type="SDL2")  # Fenstertyp für die Anzeige
        self.pyboy.set_emulation_speed(speed)
        self.lock = threading.Lock() # Mutex Erstellung
        self.running = True
    ...
    def run(self):
        ...
        try:
            ...
            print(f"Exception in emulator thread: {e}")

    def send_input(self, action):
        with self.lock:  # Sichern des PyBoy-Zustands während der Eingabe
            self.pyboy.send_input(action_map[action])

    def get_memory(self, address):
        with self.lock:  # Sichern des Speicherzugriffs
            return self.pyboy.memory[address]

    def stop_emulator(self):
        """ Stoppt den Emulator und den Thread. """
        self.running = False
        with self.lock:
            self.pyboy.stop()
...
class PokemonEnv (gym.Env):
    ...
    def __init__(self, emulator_thread):
        super().__init__()  # Initialisiere die Oberklasse gym.Env
        self.emulator = emulator_thread
    ...
    def get_done(self):
        ...
        battle_result = self.emulator.get_memory(ad.battle_result)
        if battle_result == 1:
            done = True
    ...
    def get_reward(self):
        reward = 0
        position_x = self.emulator.get_memory(ad.player_x)
        position_y = self.emulator.get_memory(ad.player_y)
    ...
    def get_observation(self):
        return self.convert_to_grayscale(self.emulator.get_observation())
...
#### Hauptschleife ####
# Initialisieren & Starten des Emulators
emulator_thread = EmulatorThread(rom_path, emulation_speed)
...
for episode in tqdm.tqdm(range(1, episodes+1), ascii=True, unit="episodes"):
    ...
    env = PokemonEnv(emulator_thread)
